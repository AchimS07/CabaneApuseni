...
export async function POST(req: NextRequest) {
  ...
  // Resolve uid/plan from Firestore if Netopia did not echo them back in data
  let resolvedUid = uid;
  let resolvedPlan = plan;
  if ((!resolvedUid || !resolvedPlan) && order.orderID) {
    const pending = await getPendingPaymentByOrderId(order.orderID);
    if (pending) {
      resolvedUid = resolvedUid ?? pending.uid;
      resolvedPlan = resolvedPlan ?? pending.plan;
      log.debug({ ntpID, orderID: order.orderID }, 'Resolved uid/plan from pendingPayments fallback');
    }
  }

  if (!resolvedUid) {
    log.warn({ ntpID, status: payment.status }, 'IPN missing uid in payment data and pendingPayments');
  ...
  try {
    switch (payment.status) {
      ...
      case STATUS_CONFIRMED: {
        if (!resolvedPlan) {
          log.warn({ uid: resolvedUid, ntpID }, 'IPN payment success but no plan found in data');
          break;
        }
        await activateSubscription(resolvedUid, resolvedPlan, ntpID, cardToken);
        if (order.orderID) await deletePendingPayment(order.orderID).catch(() => undefined);
        log.info({ uid: resolvedUid, plan: resolvedPlan, ntpID, status: payment.status }, 'Subscription activated via IPN');
        break;
      }

      case STATUS_FAILED: {
        await markSubscriptionPastDue(resolvedUid);
        log.info({ uid: resolvedUid, ntpID, status: payment.status }, 'Subscription marked past_due via IPN');
        break;
      }

      case STATUS_CANCELLED: {
        await deactivateSubscription(resolvedUid);
        if (order.orderID) await deletePendingPayment(order.orderID).catch(() => undefined);
        log.info({ uid: resolvedUid, ntpID, status: payment.status }, 'Subscription deactivated via IPN');
        break;
      }

      default: {
        // Other status codes (e.g., pending antifraud) — no subscription action needed
        log.debug({ uid: resolvedUid, ntpID, status: payment.status }, 'Unhandled Netopia IPN status');
      }
    }
  } catch (error) {
    log.error({ error, uid: resolvedUid, ntpID, status: payment.status }, 'Error handling Netopia IPN');
